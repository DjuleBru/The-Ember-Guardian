...
[CreateAssetMenu()]
public class ActiveSkillEffectSO : ScriptableObject
{
    public enum Unit {
        percentBuff,
        absoluteBuff,
    ...
    public Unit skillActiveUnit;
    public List<float> valuesByLevel; // Les valeurs pour chaque niveau
    public List<float> cooldownsByLevel; // Les valeurs pour chaque niveau
    ...
    public float GetCooldownAtLevel(int level) {
        if (level > 0 && level <= cooldownsByLevel.Count) {
            return cooldownsByLevel[level - 1]; // Retourne la valeur pour le niveau donné
        }
